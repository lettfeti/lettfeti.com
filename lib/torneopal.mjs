...
// "V04 - atNorth" -> "atNorth V04" (name with its pitch number appended).
const venuePlace = (venue) => {
...
export async function scrape(opts = {}) {
  ...
  // Cinema plan (n1motid.is/strakar/bioplan), snapshotted in data/bioplan.json:
  // { film, cinema, url, teams: { "Team name": {day,date,salur,time} } }.
  const bio = cfg.bio || null;
  const bioTeams = (bio && bio.teams) || {};
    ...
    for (const g of s.groups) {
      ...
      for (const r of g.rows) {
        ...
        teams[r.teamId] = {
          ...
          ...(isClub && { slug: teamSlug(name, cfg.club), vaktir: vaktir[name] || null, bio: bioTeams[name] || null, fixtures, nextMatch }),
          projection: projById[r.teamId],
        };
      }
    }

  // --- Consolidate squads that advanced into a day-2 country division --------
  // A squad plays the Forkeppni (day 1); based on results it is re-drawn into a
  // themed "… deildin" division (day 2) under a NEW teamId but the SAME name, so
  // it shows up twice. Merge them: the new division becomes the current face of
  // the squad (schedule/standing shown on top) and the Forkeppni is kept as
  // `prevStage` (yesterday's results shown below). Squads not yet re-drawn stay
  // as their single Forkeppni entry.
  const isForkeppni = (t) => /^Forkeppni\b/.test(t.seriesName);
  const clubByName = {};
  for (const t of Object.values(teams))
    if (t.isClub) (clubByName[t.name] = clubByName[t.name] || []).push(t);

  const canon = [];
  for (const entries of Object.values(clubByName)) {
    const fk = entries.find(isForkeppni);
    const nd = entries.find((t) => !isForkeppni(t));
    if (fk && nd) {
      nd.prevStage = {
        teamId: fk.teamId,
        series: fk.series,
        seriesName: fk.seriesName,
        lohko: fk.lohko,
        lohkoLabel: fk.lohkoLabel,
        advance: fk.advance,
        standing: {
          pos: fk.pos, played: fk.played, won: fk.won, drawn: fk.drawn,
          lost: fk.lost, gf: fk.gf, ga: fk.ga, gd: fk.gd, points: fk.points,
        },
        projection: fk.projection,
        fixtures: fk.fixtures,
      };
      // Neutralise the Forkeppni entry so slug/tracked resolution lands on the
      // canonical (new-division) squad, and it drops out of the club listing.
      fk.slug = undefined;
      fk.isTracked = false;
      fk.merged = true;
      canon.push(nd);
    } else {
      canon.push(nd || fk); // single stage — not re-drawn yet
    }
  }
  const clubTeams = canon.sort(
    (a, b) => a.seriesName.localeCompare(b.seriesName) || a.name.localeCompare(b.name)
  );
  ...
  return {
    meta: {
      ...
      bio: bio && (bio.film || bio.url) ? { film: bio.film, cinema: bio.cinema, url: bio.url } : null,
    },
    series,
    teams,
