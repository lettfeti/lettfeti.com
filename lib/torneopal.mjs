...
function parseFixtures(html, seriesName) {
  ...
  let curDate = null;
  const tokenRe =
    /<li class='title'>([^<]+)<\/li>|<a href='\/taso\/ottelu\.php\?ottelu=(\d+)'>\s*<li class='match\s*'>(.*?)<\/li>\s*<\/a>/gs;
  let tk;
...
export async function scrape(opts = {}) {
  ...
  // Fetch every series concurrently — torneopal handles parallel requests and
  // this turns ~12 sequential round-trips into ~2, slashing cold-scrape latency.
    ...
    seriesCodes.map(async (code) => {
      ...
      const [standingsHtml, fixturesHtml] = await Promise.all([
        code === cfg.seedSeries[0]
          ...
          : getPage(cfg.host, cookie, `/taso/sarja.php?turnaus=${cfg.turnaus}&sarja=${code}`),
        getPage(cfg.host, cookie, `/taso/sarja.php?turnaus=${cfg.turnaus}&sarja=${code}&ottelut=1`),
      ]);
      return { code, name, groups: parseStandings(standingsHtml), matches: parseFixtures(fixturesHtml, name) };
    })
  );
  const series = perSeries.map(({ code, name, groups }) => ({ code, name, groups }));
  const allMatches = perSeries.flatMap((s) => s.matches);
