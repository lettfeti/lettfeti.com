...
function nextUpdateMs(matches, now) {
  ...
  return (isMatchDay ? 5 : 60) * 60 * 1000;
}
...
export async function scrape(opts = {}) {
  ...
  for (const code of seriesCodes) {
    ...
    const groups = parseStandings(standingsHtml);
    ...
    for (const g of s.groups) {
      ...
      for (const r of g.rows) {
        ...
        teams[r.teamId] = {
          ...
          ...(isClub && { slug: teamSlug(name, cfg.club), roster: rosters[name] || null, fixtures, nextMatch }),
