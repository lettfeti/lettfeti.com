...
export function buildICS(team, meta, now = new Date(), opts = {}) {
  ...
  const stamp = nowStamp(now);
  // Include the Forkeppni (prevStage) games too, so a subscribed calendar keeps
  // the whole tournament — yesterday's results and today's new-division games.
  const fixtures = [...((team.prevStage && team.prevStage.fixtures) || []), ...(team.fixtures || [])];
